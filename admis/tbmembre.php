...
$errors = [];
$nom = $prenom = $post = $salaire = $contact = $adresse = $date = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Vérifications
    if (!$nom || !$prenom || !$post || !$salaire || !$contact || !$adresse || !$date) {
    ...
    if (empty($errors)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO equipes (nom_U, prenom_U, post, salaire, contact_U, Adresse_U, date, photo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$nom, $prenom, $post, $salaire, $contact, $adresse, $date, $photo]);

        header("Location: tbmembre.php");
        exit;
    }
}
$utilisateurs = $pdo->query("SELECT * FROM equipes ORDER BY id DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="fr">
...
<body>
    ...
    <h1 class="font1">
        <strong>GESTION COMPLETE DES PARTENAIRES</strong>
    </h1>
  ...
  border-bottom-right-radius: 5px; background-color:#D32F2F; display: inline-block; padding: 5px 10px; list-style: none;">
    ...
    <div id="formulaire_ajoute">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <h1 id="previewContainer">Ajouter un Partenaire </h1>
            <?php if (!empty($errors)): ?>
                <div class="error-box">
                    <ul>
            ...
            <input type="text" name="post" placeholder="Le Post" required>
            <input type="text" name="salaire" placeholder="le salaire" required>
            ...
            <tbody>
                <?php foreach ($utilisateurs as $u): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($u['post']) ?></td>
                        <td><?= htmlspecialchars($u['salaire']) ?> FCFA</td>
                        ...
                        <td><a class="btn modifier" href="modifimembre.php?id=<?= $u['id'] ?>">Modifier</a></td>
                        <td><a class="btn supprimer" href="supprimemembre.php?id=<?= $u['id'] ?>" onclick="return confirm('Confirmer la suppression ?')">Supprimer</a></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
